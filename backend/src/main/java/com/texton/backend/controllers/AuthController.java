...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateAndGetToken(@RequestBody LoginRequest authRequest) {
        ...
        // Hardcoded superadmin handling (unchanged)
        ...
        // ✅ FIX: Extract principal properly
        UserDetails userDetails = (UserDetails) authentication.getPrincipal();

        // ✅ Fetch real DB user to check role
        User realUser = userRepository.findByUsername(userDetails.getUsername())
                .orElseThrow(() -> new UsernameNotFoundException("User not found in DB"));

        // ✅ Check role based on UI selection
        if (!realUser.getRole().name().equalsIgnoreCase(authRequest.getUser_type())) {
            throw new SecurityException("Access denied. Portal does not match user role (" + realUser.getRole().name().toLowerCase() + ").");
        }

        // ✅ Generate token using correct username
        String token = jwtService.generateToken(userDetails.getUsername());

        return ResponseEntity.ok(Map.of(
                "access_token", token,
                "role", realUser.getRole().name().toLowerCase()
        ));
    }
